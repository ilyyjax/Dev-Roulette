...
function spinWheel() {
  ...
  let spinAngle = Math.random() * Math.PI * 6 + Math.PI * 6;
  ...
  function animate(time) {
    ...
    if (progress < 1) {
      ...
      let velocityFactor = Math.sin(progress * Math.PI);
      ...
      let pointerAngle = (Math.PI / 2 - angle + 2 * Math.PI) % (2 * Math.PI);
      let index = Math.floor(pointerAngle / arc);
      let result = options[index];

      if (result === "Sell") {
        spinning = false;
        overlay.classList.add("show"); // ✅ YOU LOSE SCREEN ALWAYS SHOWS
      } else {
        // Remove one Buy
